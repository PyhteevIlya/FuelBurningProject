...
const DocsPage = () => {
  ...
  const getRaschet = (data: any) => {
    setLoading(true);
    request('https://localhost:7050/LiquidController2/Raschet').then(result => {
      console.log(result);
  ...
  const removeHandler = (id: number) => {
    ...
    request(`https://localhost:7050/LiquidController2/Remove?id=${id}`, { method: 'DELETE' }).then(result => {
      console.log(result);
  ...
  React.useEffect(() => {
    getRaschet({});
    document.title = "Расчет жидкого топлива";
  }, []);
  ...
  const prodsite: ColumnsType<never> = [
...

    {
      ...
      render: (value, record, index) =>
        <>
          <Link to={`/edit_raschet_liquid/${record.id}`}>
            <EditFilled />
          </Link>{' / '}
          {index !== 0 && (
            <>
          ...
          <Link to={`/rezult_liquid/${record.id}`}>
            <PlayCircleFilled />
          </Link>
        </>
  ...
  return (
    <div>
      <Space direction="vertical" style={{ marginBottom: '10px' }}>
        <Link to="/create/create_raschet_liquid">
