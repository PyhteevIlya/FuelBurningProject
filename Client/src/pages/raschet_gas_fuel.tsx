...
const DocsPage = () => {
  ...
  const getRaschet = (data: any) => {
    setLoading(true);
    request('https://localhost:7050/HomeController2/Raschet').then(result => {
      console.log(result);
  ...
  const removeHandler = (id: number) => {
    ...
    request(`https://localhost:7050/HomeController2/Remove?id=${id}`, { method: 'DELETE' }).then(result => {
      console.log(result);
  ...
  React.useEffect(() => {
    getRaschet({});
    document.title = "Расчет газообразного топлива";
  }, []);
  ...
  const prodsite: ColumnsType<never> = [
...

    {
      ...
      render: (value, record, index) =>
        <>
          <Link to={`/edit_raschet_gas/${record.id}`}>
            <EditFilled />
          </Link>{' / '}
          {index !== 0 && (
            <>
  ...
  return (
    <div>
      <Space direction="vertical" style={{ marginBottom: '10px' }}>
        <Link to="/create/create_raschet_gas">
